---
title: "Statistical Learning - Group 16 - Final Presentation"
...
- occupation class
- Expiration weightage (numeric, scaled)
```{r message=FALSE, warning=FALSE, include=FALSE}
  ...
  coupon_data$occupation_class[which(coupon_data$occupation %in% 
                                       c("Community & Social Services","Construction & Extraction","Healthcare Support"
                                         ,"Life Physical Social Science"))] <- "Technicians & prof"
  coupon_data$occupation_class[which(coupon_data$occupation %in% 
...
p1 <- ggplot(cleaned_data, aes(x=destination, fill=Y)) +
    geom_bar(stat="count")
g1 <- ggplot(cleaned_data, aes(x=destination, fill=Y)) +
    geom_bar(position="fill")

#passanger 
p2 <- ggplot(cleaned_data, aes(x=passanger, fill=Y)) +
  geom_bar(stat="count")
g2 <- ggplot(cleaned_data, aes(x=passanger, fill=Y)) +
  geom_bar(position="fill")

#weather
p3 <- ggplot(cleaned_data, aes(x=weather, fill=Y)) +
  geom_bar(stat="count")
g3 <- ggplot(cleaned_data, aes(x=weather, fill=Y)) +
  geom_bar(position="fill")
...
g6 <- ggplot(cleaned_data, aes(x=maritalStatus, fill=Y)) +
  geom_bar(position="fill")+
  theme(axis.text.x = element_text(angle = 15, hjust = 1))
...
g7 <- ggplot(cleaned_data, aes(x=education, fill=Y)) +
  geom_bar(position="fill")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
grid.arrange(p7,g7,ncol=2)
```
...
p8 <- ggplot(cleaned_data, aes(x=occupation_class, fill=Y)) +
  geom_bar(stat="count")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
g8 <- ggplot(cleaned_data, aes(x=occupation_class, fill=Y)) +
  geom_bar(position="fill")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
grid.arrange(p8,g8,ncol=2)
...
p9 <- ggplot(cleaned_data, aes(x=Bar, fill=Y)) +
  geom_bar(stat="count")
g9 <- ggplot(cleaned_data, aes(x=Bar, fill=Y)) +
  geom_bar(position="fill")

#CoffeeHouse                                       
p10 <- ggplot(cleaned_data, aes(x=CoffeeHouse, fill=Y)) +
  geom_bar(stat="count")
g10 <- ggplot(cleaned_data, aes(x=CoffeeHouse, fill=Y)) +
  geom_bar(position="fill")
...
p13 <- ggplot(cleaned_data, aes(x=direction_same, fill=Y)) +
  geom_bar(stat="count")
g13 <- ggplot(cleaned_data, aes(x=direction_same, fill=Y)) +
  geom_bar(position="fill")
...
#direction_opp                                              
p15 <- ggplot(cleaned_data, aes(x=direction_opp, fill=Y)) +
  geom_bar(stat="count")
g15 <- ggplot(cleaned_data, aes(x=direction_opp, fill=Y)) +
  geom_bar(position="fill")

grid.arrange(p9,g9, p10,g10, p13,g13, p15,g15, ncol=2) #, p11, p12,
...
- 'occupation' will be replaced by 'occupation_class' in the model.  
- 'expiration' replaced by numeric variable.
...
Keeping **insignificant variables** in the model may contribute to overfitting. 
Eliminated **manually, using stepwise regression and penalized regression methods**.
...
We tried to apply Lasso and Elastic Net to find a reduced set of variables resulting to an optimal performing mode (penalty for having too many variables - regularization).
...
- Remove potential outliers (Robust Logistic Regression, remove outliers within categorical variables and combinations of categorical variables)
- Random forest and boosting models to capture non-linearities and interactions between the variables
- Another option to apply Generalised Additive Models.


# RANDOM FOREST

- Non-linear classifier, made up of an ensemble of decision trees that are independent of each other and each will predict the outcome variable using their own set of rules
- Random Forest Classifier is more of Accuracy focused algorithm and is best till it’s used with proper fit, else it gets overfit quickly
- Random Forest Classifier performs better with more categorical data than numeric and logistic regression is a little confusing when comes to categorical data
...
cleaned_data_raw_columns <- read.csv("https://raw.githubusercontent.com/rijinbaby/Statistical-Learning/main/cleaned_data_raw_columns.csv")
  ...
  dataset_rf$expiration_weightage <- scale(as.numeric(dataset_rf$expiration_weightage), center = FALSE)
...
- Trees are grown sequentially & each tree uses information from previous tree
...
- Interested to see if the drivers could be segmented easily based on the data present
...
```{r echo=FALSE}
set.seed(123) 
sample_clust <- cleaned_data_clust[sample(nrow(cleaned_data_clust), 1000),]
```
...
library(fossil)
# Rand Index between k-medoids and agglomerative clustering.
rand.index(pam_fit$clustering, groups) 

# Rand Index between k-medoids and true labels
rand.index(pam_fit$clustering, as.integer(sample_clust$Y))

# Rand Index between agglomerative clustering and true labels
rand.index(groups, as.integer(sample_clust$Y))
```
